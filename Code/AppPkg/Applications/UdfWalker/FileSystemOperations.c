  ...
  )
{
  ...
  //printf ("[%4llx] LBA [%4llx] %lld Size [%x]\n", count++, Offset / ISOFS_BLOCK_SIZE, Offset / ISOFS_BLOCK_SIZE, Size);
  return (fread (Buffer, 1, Size, pFile) != Size ? EFI_DEVICE_ERROR : EFI_SUCCESS);
}
  ...
  )
{
  ...
  return _ftelli64 (pFile) / BlockSize;;
}
#else
STATIC
EFI_STATUS
...
{
  static UINT64 count;
  FileHandleSetPosition (FileHandle, Offset);
  //printf ("[%4llx] LBA [%4llx] %lld Size [%x]\n", count++, Offset / ISOFS_BLOCK_SIZE, Offset / ISOFS_BLOCK_SIZE, Size);
  return FileHandleRead (FileHandle, &Size, Buffer);
}
  ...
  )
{
  ...
  return DivU64x32 (FileSize, BlockSize);
}
#endif
...
#elif defined(WIN_CONSOLE)
  BlockSize = ISOFS_BLOCK_SIZE;
  EndLBA = InternalDiskLBA (pFile, BlockSize);
#else
  BlockSize = ISOFS_BLOCK_SIZE;
  EndLBA = InternalDiskLBA (FileHandle, BlockSize);
#endif
  ...
  )
{
  ...
  for (; SeqStartBlock < SeqEndBlock && !StopSequence; SeqStartBlock++) {
    ...
    Status = InternalReadDisk (
               pFile,
               MultU64x32 (SeqStartBlock, BlockSize),
#else
    Status = InternalReadDisk (
               FileHandle,
               MultU64x32 (SeqStartBlock, BlockSize),
    ...
    DescriptorTag = (UDF_DESCRIPTOR_TAG *)Buffer;
...
#elif defined(WIN_CONSOLE)
  Volume->MainVdsStartLocation = (UINT64)ExtentAd->ExtentLocation * 0;
#else
  Volume->MainVdsStartLocation = (UINT64)ExtentAd->ExtentLocation * 0;
#endif
  ...
  )
{
  ...
  DescriptorTag = (UDF_DESCRIPTOR_TAG *)FileEntryData;
  ...
  )
{
  ...
  RecordingFlags = (UDF_FE_RECORDING_FLAGS)GET_FE_RECORDING_FLAGS (FileEntryData);
...
#if defined(UDK_ORIGINAL)
#elif defined(WIN_CONSOLE)
        ReadFileInfo->ReadLsn = (ReadFileInfo->ReadLsn == 0 ? Lsn : ReadFileInfo->ReadLsn);
#else
        ReadFileInfo->ReadLsn = (ReadFileInfo->ReadLsn == 0 ? Lsn : ReadFileInfo->ReadLsn);
#endif
        break;
...
#elif defined(WIN_CONSOLE)
    DuplicateFe (Volume, Parent->FileEntry, &File->FileEntry);
#else
    DuplicateFe (Volume, Parent->FileEntry, &File->FileEntry);
#endif
...
#elif defined(WIN_CONSOLE)
  Status = ReadVolumeFileStructure (pFile, Volume);
#else
  Status = ReadVolumeFileStructure (FileHandle, Volume);
#endif
  if (EFI_ERROR (Status)) {
    return Status;
  }
...
#elif defined(WIN_CONSOLE)
  Status = FindFileSetDescriptor (pFile, Volume);
#else
  Status = FindFileSetDescriptor (FileHandle, Volume);
#endif
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  )
{
  ...
  DescriptorTag = (UDF_DESCRIPTOR_TAG *)ReadBuffer;
    ...
    *FileNamePointer = L'\0';
    if (FileName[0] == L'\0') {
      ...
      if (Root == NULL) {
        ...
        Status = InternalFindFile (BlockIo,
                                   ...
                                   Volume,
#elif defined(WIN_CONSOLE)
        Status = InternalFindFile (pFile,
                                   Volume,
#else
        Status = InternalFindFile (FileHandle,
                                   Volume,
#endif
                                   (CHAR16 *)L"\\",
                                   &PreviousFile,
...
#elif defined(WIN_CONSOLE)
        DuplicateFe (Volume, Root->FileEntry, &File->FileEntry);
#else
        DuplicateFe (Volume, Root->FileEntry, &File->FileEntry);
#endif
        if (File->FileEntry == NULL) {
    ...
    if (FE_ICB_FILE_TYPE (File->FileEntry) == UdfFileEntrySymlink) {
      ...
      Status = ResolveSymlink (BlockIo,
                               DiskIo,
#elif defined(WIN_CONSOLE)
      Status = ResolveSymlink (pFile,
#else
      Status = ResolveSymlink (FileHandle,
#endif
                               Volume,
                               &PreviousFile,
                               FileEntry,
...
#elif defined(WIN_CONSOLE)
      DuplicateFe (Volume, PreviousFile.FileEntry, &File->FileEntry);
#else
      DuplicateFe (Volume, PreviousFile.FileEntry, &File->FileEntry);
#endif
      if (File->FileEntry == NULL) {
  ...
  )
{
  ...
  LogicalVolInt = (UDF_LOGICAL_VOLUME_INTEGRITY *)AllocatePool (ExtentAd->ExtentLength);
